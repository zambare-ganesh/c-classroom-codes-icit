...
void print_array(Student *ps, int length)
{
	...
	for(i = 0; i < length; ++i)
	{
		fprintf(stdout, "%3hd %-15s %c %02hd-%02hd-%02hd", ps[i].rollno, ps[i].name, ps[i].gender, 
...
void scan_array(Student *ps, int length)
{
	...
	for(i = 0; i < length; ++i)
	{
		fscanf(stdin, "%hd%s%*c%c%hd%hd%hd", &ps[i].rollno, ps[i].name, &ps[i].gender, 
									&ps[i].dob.day, &ps[i].dob.month, &ps[i].dob.year);
		for(j = 0; j < 4; ++j)
		{
			fscanf(stdout, "%hd", &ps[i].marks[j]);
		}
	}
}
