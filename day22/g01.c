...
int main( int argc, char *argv)
{
	...
	//print info
	//fprintf(stdout, "%3hd %-15s%02hd-%02hd-%02hd", s.rollno, &s.name[0], s.dob.day, s.dob.month, s.dob.year);
	fprintf(stdout, "%3hd %-15s%02hd-%02hd-%02hd", s.rollno, s.name, s.dob.day, s.dob.month, s.dob.year);
	for(i = 0; i < 4; ++i)
	{
		fprintf(stdout, "%3hd", s.marks[i]);
	}
	fprintf(stdout, ": %3hd\n", s.total_marks);
